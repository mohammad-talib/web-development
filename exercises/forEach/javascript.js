...
function onlyEvenValues(arr){
    var newArr = [];
    arr.forEach(x => {if(x % 2 == 0){newArr.push(x)}});
    return newArr;
}
...
function showFirstAndLast(arr){
  ...
  arr.forEach(x => newArr.push(x[0]+x[x.length-1]));
  return newArr;
}
...
* Write a function called addKeyAndValue which 
  accepts an array of objects, a key, and a value and returns the array passed 
